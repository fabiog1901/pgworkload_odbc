...
"""
    ...
    def init(self, conn: pyodbc.Connection):
    ...
    # conn is an instance of a pyodbc connection object
    # conn is set with autocommit=True, so no need to send a commit message
    def read(self, conn: pyodbc.Connection):
        with conn.cursor() as cur:
            cur.execute(
                "select * from transactions where lane = ? and id = ?",
                (self.lane, self.uuid),
            )
            cur.fetchone()

    def txn1_new(self, conn: pyodbc.Connection):
    ...
    def txn2_verify(self, conn: pyodbc.Connection):
        conn.autocommit = False
        
        with conn.cursor() as cur:
            cur.execute(
                "select * from ref_data where my_sequence = ?",
    ...
    def txn3_finalize(self, conn: pyodbc.Connection):
        conn.autocommit = False
